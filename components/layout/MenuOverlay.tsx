    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="fixed inset-0 z-40 flex flex-col justify-between bg-midnight px-6 pb-10 pt-28 sm:px-10"
        >
          <nav className="flex flex-col gap-2">
              ...
              return (
                ...
                >
                  <Link
                    ...
                    onClick={onClose}
                    data-cursor="link"
                    className={`block border-b border-cool-grey/10 py-4 font-display text-4xl uppercase tracking-wide transition-colors sm:text-6xl ${
            ...
            <motion.div
              initial={{ opacity: 0, y: 24 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.15 + NAV_LINKS.length * 0.06, duration: 0.5, ease: "easeOut" }}
            >
              <Link
                href="/contact"
                onClick={onClose}
                data-cursor="link"
                className="mt-6 inline-block bg-crimson px-8 py-3 font-display text-sm uppercase tracking-[0.2em] text-midnight transition-opacity hover:opacity-90"
              >
                Join Now →
              </Link>
            </motion.div>
